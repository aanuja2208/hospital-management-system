...
# SPA middleware: serve index.html for non-API, non-static GET requests that result in 404
class SPAMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        response = await call_next(request)
        path = request.url.path
        # If it's a 404 for a non-API, non-static GET request, serve the SPA
        if (
...
# SPA fallback (added AFTER CORS so it runs before CORS in the middleware stack)
